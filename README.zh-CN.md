...
将 ZIP 解压到一个新目录，然后运行 `tpt-zh-omnipack.exe`。这是便携版，默认不会安装文件关联。跨版本使用前请备份重要存档。

当前 Windows 可执行文件没有数字签名，因此 Windows 可能显示未知发布者提示；运行前请先使用 ZIP 旁的 SHA-256 文件核对下载内容。

## 源码与许可证

项目整体依据 GNU GPL v3 发布。`LICENSES` 目录内包含字体、静态库及其他第三方组件的许可证和通知，源码来源记录见 `SOURCE-AND-LICENSES.zh-CN.md`。

公开下载只包含玩家程序和必要文档，不包含测试样本、测试报告、调试符号、偏好文件、截图、日志或个人存档。
