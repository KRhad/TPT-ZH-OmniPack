...
tool = GetOption('tool')
isX86 = platform.machine() in ["amp64", "AMD64", "i386", "i686", "x86", "x86_64"]
...
def findLibs(env, conf):
	...
	if msvc:
		if not conf.CheckHeader('fftw3.h') or not conf.CheckHeader('sched.h') or not conf.CheckHeader('zlib.h'):
