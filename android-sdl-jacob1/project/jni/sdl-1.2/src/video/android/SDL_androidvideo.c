...
JAVA_EXPORT_NAME(DemoRenderer_nativeResize) ( JNIEnv*  env, jobject  thiz, jint w, jint h, jint keepRatio )
{
	...
	recalculateTouchscreenCalibration();
...
void recalculateTouchscreenCalibration()
{
	...
	/*
	 * TPT renders its UI and bitmap font into a 642x424 software surface.
	 * Stretching that surface to an arbitrary Android display size gives every
	 * source pixel an uneven footprint and destroys the carefully rasterised
	 * 2-bit font edges.  Use the largest exact integer scale that fits instead.
	 * The SDL 1.2 renderer centres this logical window in the physical surface.
	 */
	if( SDL_ANDROID_sFakeWindowWidth > 0 && SDL_ANDROID_sFakeWindowHeight > 0 )
	{
		int scaleX = SDL_ANDROID_sRealWindowWidth / SDL_ANDROID_sFakeWindowWidth;
		int scaleY = SDL_ANDROID_sRealWindowHeight / SDL_ANDROID_sFakeWindowHeight;
		int integerScale = scaleX < scaleY ? scaleX : scaleY;
		if( integerScale >= 1 )
		{
			SDL_ANDROID_sWindowWidth = SDL_ANDROID_sFakeWindowWidth * integerScale;
			SDL_ANDROID_sWindowHeight = SDL_ANDROID_sFakeWindowHeight * integerScale;
		}
	}
	...
	__android_log_print(ANDROID_LOG_INFO, "libSDL", "Touchscreen fake resolution is %dx%d 43Ratio %d",
			SDL_ANDROID_TouchscreenCalibrationWidth, SDL_ANDROID_TouchscreenCalibrationHeight,
			SDL_ANDROID_ScreenKeep43Ratio);
}
