...
The Android ARM64 direct port builds the same simulation, stable element IDs, Chinese font, and OPS save code into a landscape SDL application. See [docs/ANDROID_PORT.md](docs/ANDROID_PORT.md). A signed APK has been exercised in an isolated Android 15 emulator for touch drawing, bilingual IME search, in-app language restart, periodic-table and description flows, local save creation and reopen, and several display/DPI profiles. Physical ARM64-device performance, background lifecycle, and long-duration stress remain separate release gates.
