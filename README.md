...
This repository is an unofficial GPL-3.0, content-first free-sandbox integration based on The Powder Toy `100.0.399`. It has no player tasks, achievements, technology tree, discovery progression, or forced element unlocks: every enabled material is directly available. The current development tree includes Simplified Chinese and English support plus 55 stable-ID OmniPack elements across metallurgy, local ecology, advanced chemistry, controlled nuclear, and the first periodic-table batch.

The 118-element source map reuses 26 existing pure-element implementations and fixes stable IDs `370..461` for the remaining 92. A compiled standard long-form periodic-table panel now supports bilingual/name/symbol/atomic-number/identifier search, state/radioactivity/metal-class filters, category colours, and an expandable f-block. Seven new noble gases are implemented at their fixed IDs, so 33 of 118 periodic mappings are currently placeable; the other 85 cells are visibly marked as pending rather than represented by empty shell elements.
