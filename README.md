...
The development branch has committed clean-build, automated registration, bilingual content, gameplay, disabled-module, OPS roundtrip, and short stress-smoke evidence for the first environmental batch. It is **not release-ready**: trusted GUI/DPI checks, formal 600-second stress sampling, the two-hour long run, full third-party license closure, an authorized public source remote, tag, and public release remain incomplete. The existing `origin` is a historical Chinese-fork remote and must not be represented as the public source for this candidate.
