...
The development branch has automated registration, bilingual content, gameplay, disabled-module, OPS roundtrip, and short stress-smoke evidence. It is **not release-ready**: the new environmental batch still needs its final committed clean-build evidence, trusted GUI/DPI checks, formal 600-second stress sampling, the two-hour long run, full third-party license closure, an authorized public source remote, tag, and public release. The existing `origin` is a historical Chinese-fork remote and must not be represented as the public source for this candidate.
