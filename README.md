...
The periodic table is the unified picker for elemental substances, isotopes, and inorganic compounds. Organic materials and alloys/engineering materials have separate menu sections; upstream TPT elements and existing OmniPack special content stay in their rc8 menu positions. The standard menus contain 301 materials, while 165 OmniPack elemental, isotopic, and inorganic tools are available through periodic detail pages without changing their IDs, physical states, reactions, searchability, Lua access, or favorite support.

The 118-element source map reuses 26 existing pure-element implementations and fixes stable IDs `370..461` for the remaining 92; all 118 mappings are placeable. Clicking an element cell opens a scrollable picker that first shows the localized element description, followed by the elemental substance, implemented isotopes, and related inorganic compounds. Search accepts bilingual element/material names, symbols, atomic numbers, and formulas; internal identifier matching remains compatible without exposing that developer term in the normal prompt. The panel retains state/radioactivity/metal-class filters, category colours, and an expandable f-block.

The development branch has committed clean-build, automated registration, bilingual content, gameplay, disabled-module, OPS roundtrip, and content-freeze evidence. The `f970a534` rc9 candidate passed ordinary/symbol package audits and all six S15-S20 30-second stability gates. The source, mod-reference, font, resource, and static-library license closure also passes with unknown sources excluded, but the project is **not release-ready**: trusted GUI/DPI checks, the separate two-hour long run, an authorized public source remote, tag, signing, and public release remain incomplete. The existing `origin` is a historical Chinese-fork remote and must not be represented as the public source for this candidate.
