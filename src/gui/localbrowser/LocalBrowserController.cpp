...
{
	StringBuilder desc;
	desc << "确定要删除 " << browserModel->GetSelected().size() << " 个图章吗？";
...
void LocalBrowserController::removeSelectedC()
{
	...
	public:
		...
		bool doWork() override
		{
			...
			{
				notifyStatus(String::Build("正在删除图章 [", saves[i].FromUtf8(), "]……"));
	...
	new TaskWindow("正在删除图章", new RemoveSavesTask(this, selected));
}
...
void LocalBrowserController::RenameSelected()
{
	...
	new TextPrompt("重命名图章", "请输入图章的新名称：", "", "[新名称]", false, { [this, save](const String &newName) {
		if (newName.length() == 0)
		{
			new ErrorMessage("图章重命名失败", "必须填写文件名。");
			return;
		}
