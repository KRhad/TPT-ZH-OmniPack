...
Phase 8：Windows x64 内部测试包完成；下一阶段为人工 UI、OPS 往返与固定压力样本。
...
1. 执行测试包中的手动 UI、中文/英文切换和正常/只读存档加载检查。
2. 为已实现模块建立禁用状态下的 OPS 往返样本，并执行正常/只读保存行为的实际客户端验证。
3. 设计并测试可逆的兼容占位策略，不在未验证前转换或删除存档粒子。
4. 建立冶金、化学、生态和核工业生产线、官方存档和性能固定样本。
