...
omnipack_active_elements=81
total_active_elements=276
registered_slots=462
reserved_slots=186
enabled_content_modules=4
periodic_elements_placeable=59
periodic_elements_remaining=59
...
## Phase 2/3：周期表 UI、稀有气体、碱金属、碱土金属、硼族、碳族、氮族与氧族批次

- 118 行来源映射生成编译时周期表模型；标准长式面板支持中英文名、符号、原子序数、identifier、状态、放射性和金属类别筛选，f 区可展开；
- 周期表只是直接选择入口，不读取存档进度，不增加解锁或任务状态；未实现的 59 格保留正确位置并明确显示待实现；
...
- 共享 `OmniPeriodic.cpp` 只做 `3x3` 局部检查，放电、低温换热、六类主族反应和衰变共享 `1024` 次/帧预算；火焰与高能光子寿命有限；
- 真实客户端 Lua 回归：`OMNI_PERIODIC_STATUS=PASS`、33 个新元素、59 个已实现周期映射、六个主族各 6 个成员，压力帧事件恰为 1024；
- 周期 OPS：3 进程、2 重启、2 加载、39 粒子、47 字段断言、33 个直接大于 255 的类型与携带字段通过；
- 模块/Lua 回归确认从 `HE` 到 `LV` 的各批代表项均可直接选择且 Lua 仍优先分配 ID `255`；
- 全新 `build-periodic-oxygen-group-final-clean` Windows x64 Release 构建 `546/546` 通过；最终生成目录重链后 EXE SHA-256 `F074CED19649A849A8DF919D4E33B11D5C5936A5551BBE10C7A52DFF80841763`，Meson static `21/21`、Python `152/152`（0 skip）通过；
- clean EXE 已复跑四模块、完整/简化生态、周期、模块、六类 OPS 与 mixed OPS：合计 21 个进程、14 次重启、14 次加载验证；六类为 118 粒子/167 字段断言，周期用例含 33 个大于 255 的直接类型。
