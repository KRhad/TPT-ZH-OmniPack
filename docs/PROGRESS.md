...
- 277 个当时已实现的 OmniPack 登记项菜单代号均为唯一四位大写字母；当前审计契约允许未来使用三至四位大写字母，现有 identifier、代号、稳定 ID、周期原子序数及核素科学符号未变；玩家说明以材料名开头，图鉴隐藏稳定 ID、源码 commit 与实现/测试门禁字段；
- 周期表入口与元素搜索按钮统一放入最右侧竖列，分别位于 `WINDOWH-48` 和 `WINDOWH-32`，分类按钮从 `WINDOWH-64` 向上排列；元素横条恢复到 `WINDOWW-56` 的正常宽度，并按完整按钮步长滚动。旧 `P` 入口曾在 96 DPI、游戏 2× 缩放下完成真实点击；当前入口已改为专属像素图标，位置、点击回调和右栏避让静态通过，但新图标桌面视觉需重新验证。扩展气体继续使用与官方 `TYPE_GAS` 相同的 `FIRE_BLEND + DECO_FIRE` 云团底层，其完整视觉矩阵仍为 `not_tested`；
...
- 周期表专属图标与搜索按钮统一到最右侧竖列，分类菜单为二者预留两个固定行位；元素按钮的绘制区和滚动点击框均止于右栏之前；
- `build-0.7.0-dev-private-static-clean` 增量编译和链接通过；EXE 为 337,399,229 字节，SHA-256 `2D9A8F746888371FA2C098ED12E5DA85BBE6A47458DB0A9879C3F95A3747BB11`；
- 旧 `P` 入口的 Python `210/210`、Meson static `32/32` 和 96 DPI/2× 实屏点击证据位于 `artifacts/gui/0.7.0-dev/20260802T021500Z-right-column/`；该历史证据只证明位置和点击层级，不用于证明新图标视觉；
- 当前专属图标源码从新目录完成 `771/771`，Meson `32/32`、Python `218/218`（2 skip），真实客户端模块、周期 118/118 和 mixed OPS 回归通过。图标在 100%/125%/150% DPI 下的桌面观感仍为 `not_tested`。

## 粉末与固体碰撞修复

- 碰撞矩阵不再允许 `TYPE_PART` 仅凭重量优势置换 `TYPE_SOLID`；气凝胶、石墨、碳纤维复合材料等低密度固体现在会稳定阻挡普通粉末；
