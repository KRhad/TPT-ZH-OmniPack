...
Phase 4：局部生态首批完成；下一阶段为 Phase 5：扩展能源与核工业设计。
...
Phase 2、已完成的 Phase 3 冶金/基础化学和 Phase 4 局部生态的主要变更：
...
- 官方活动元素名称登记：195/195；冶金/化学/生物元素中英文正式名称登记：23/10/8
...
Phase 2 没有新增粒子更新函数，因此没有新增每帧粒子成本。Phase 3 冶金反应路径限定为局部 `3x3` 搜索；合金/炼钢共享每帧 2,048 次成功反应预算。基础化学同样只用局部 `3x3` 搜索，所有成功反应共享每帧 1,536 次预算。Phase 4 局部生态同样固定为 `3x3`，共享每帧 1,024 次成功事件预算，且完整和简化模式都不创建新粒子。三个模块均不扫描全粒子表；Lua 回归中客户端保持响应。FPS、内存、粒子增长和大型生产线压力数据仍为 `NOT RUN`，将在 Phase 7 建立固定样本。
...
- 关闭冶金、化学或生物模块后加载含模块元素的中文警告、只读和占位路径尚未实现；这是 Phase 7 存档兼容测试的发布阻塞项。
...
1. 为固定 ID `328..359` 制定扩展能源与核工业的元素、来源和性能边界，先完成设计审计再写运行规则。
2. 扩展图鉴登记字段与反应数据结构，添加配方、生产方法、用途和危险说明。
3. 实现禁用模块存档的中文警告和兼容占位策略，再进行 OPS 往返存档测试。
4. 建立冶金、化学和生态生产线、官方存档和性能固定样本。
