...
- 周期表 `P` 与元素搜索按钮现统一放入最右侧竖列，分别位于 `WINDOWH-48` 和 `WINDOWH-32`，分类按钮从 `WINDOWH-64` 向上排列；元素横条恢复到 `WINDOWW-56` 的正常宽度，并按完整按钮步长滚动。96 DPI、游戏 2× 缩放下已真实点击 `P` 打开 118 格周期表，截图 SHA-256 为 `70458914...E248`（主界面）和 `3EF4E394...1A2C`（点击后）；完整 DPI 矩阵仍未执行。扩展气体继续使用与官方 `TYPE_GAS` 相同的 `FIRE_BLEND + DECO_FIRE` 云团底层，其完整视觉矩阵仍为 `not_tested`；
...
- `P` 与搜索按钮统一到最右侧竖列，分类菜单为二者预留两个固定行位；元素按钮的绘制区和滚动点击框均止于右栏之前；
...
- Python `210/210`、Meson static `32/32` 通过；96 DPI、游戏 2× 缩放下主界面实屏可见最右列 `P`，真实鼠标点击后打开 118 格周期表；
- 证据目录为 `artifacts/gui/0.7.0-dev/20260802T021500Z-right-column/`。本项只关闭按钮位置和点击层级缺陷，不把 125%/150% DPI、完整双语窗口或气体视觉冒充为已完成。
