...
- 空目录 Windows x64 Release 构建 `502/502`，Meson `13/13`，Python `77/77`，0 skip；已剥离 EXE 的模块/四反应引擎 Lua 回归 `6/6`。
...
- 候选冻结后又增加只读压力证据判定器和机器可读报告门禁；开发 HEAD `d3419e7b` 的 Meson 为 `14/14`、Python 为 `86/86`。这些工具不改变上述候选 EXE，但进入下一候选前仍需重新封包并记录新清单提交。
