...
- `f8fa2b97` 的 0.2 用途审计基线覆盖 48/48 元素：当时为 23 项仅直接放置、33 项缺跨模块联动、4 项缺回收和 1 项未验证声明。实现提交 `98affcd7` 的增量另列如下，不回写为 0.1 候选证据。
...
- 传感、过滤、延迟、计数、存储、阀门、加料、排废、停机、报警和联锁共 11 类能力全部复用官方电子元件；17 个直接复用源码有固定哈希，自动化预留 ID `392..423` 未占用。
- 9 个真实自动化 OPS 和 6 项挑战由候选 EXE 生成、重载并通过：总断言 95，停止后事件增量 0；同一 EXE 还刷新并通过 0.2 示例 `7/7`、教程 `8/8`。
- 48 个模组元素说明已统一为英文 `Name: ...`、中文 `名称：……`；登记表、名称键和两份语言包逐字一致，fail-closed 负向测试覆盖缺前缀和双源漂移。
- 文案所需全角冒号纳入固定字体后，字体为 `C13C3D0...66B0`、14,626 字形、2,590 字符覆盖；Python `121/121`、Meson `16/16`。
- `0.3.0-dev/local-dev` 普通包与符号包审计通过；普通 ZIP `BB57D87F...07EA`，EXE `FBCA1387...0FF2`。相同输入重复封包逐字节一致；clean build 普通 EXE 也复现，detached symbols 跨 build 目录哈希不同并明确记为 `false`。
...
Phase 2 没有新增粒子更新函数。冶金、化学、生态和核工业反应均限定为局部 `3x3`，分别共享每帧 2,048、1,536、1,024 和 512 次成功事件预算，不扫描全粒子表。0.1 十项、0.2 四项以及 0.3 S11/S12 正式 `60+600` 秒压力均已有 FPS、内存、粒子、事件/信号、停止/恢复和双 OPS 原始序列；当前通过样本的有限观察 `unbounded_growth=false`、`memory_leak_suspected=false`。这仍不替代 1.0.0 的综合两小时长跑。
...
- 当前 Windows x64 候选 ZIP 已通过隐私/完整性审计，0.1 十项、0.2 四项及 0.3 两项正式性能门禁通过；UI 人工复核、跨来源迁移及综合两小时长跑仍缺失，不得称为正式公开发布。
...
4. 保持已通过的 0.2/0.3 示例、挑战和压力证据不可变；本地进入 0.4 炼金探索开发，但不把后续开发冒充为 `GATE-020/030-RELEASE`，不创建对应 tag。
...
- 官方能力、场景与审计：`52544032509f0b6e6888c6322a1c385e2f62ee3c`
- 双语元素自识别说明：`c75aeaf18103ed6cb6eddc844d80b7a3a87045fc`
- 包源码与两代内容证据：`dafec4d082fead6da49cb7e236f87262236096d6`
- 持续官方信号压力夹具：`522dc3bc10c363bfd73333928f596a482b60a274`
- 精确工件与 run ID：`docs/PHASE_0_3_EVIDENCE.md`
