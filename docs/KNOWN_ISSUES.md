...
5. 十个固定压力场景和隔离采样工具已建立。最初 2 秒烟测全部能生成 FPS、1% low、工作集、粒子和 OPS 数据；第一次完整样本因 Lua 连续占用触发“脚本无响应”而失败，已在 `4f5c07f9` 改为逐 UI tick 采样。当前候选的十个 `60+600` 秒样本均完成，事件计数、停止/恢复和独立工件评估全部通过，十项 `performance_gate_pass=true`；有限观察不证明长期有界，两小时长跑仍未执行。
