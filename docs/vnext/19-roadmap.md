...
scaffold, shared contracts and first Rusanov uniform debug probe are now
...
| 3 | AtmosphereBench | 1.0.5 IN_PROGRESS: Rusanov has one strict-double 1D uniform debug probe; HLLE/LBM remain registered-only |
...
15. `physical-scale-atmospherebench`: IN PROGRESS in 1.0.5; candidate scale,
   strict-double target, contract-only fixture and one isolated Rusanov uniform
   debug probe exist. Scale/time/solver selection remains RED.
...
| Next stage? | Add one non-uniform deterministic Rusanov case at a time while HLLE/LBM remain registered-only. Production OmniAtmosphere remains unauthorized. |
