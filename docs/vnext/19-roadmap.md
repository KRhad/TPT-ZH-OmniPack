...
the uncoupled hybrid component checkpoint now proves advective-CFL constant-
pressure transport and a whole-case HLLC Sod route, while explicitly retaining
router/reflux/event-local coupling as unimplemented;
...
15. `physical-scale-atmospherebench`: IN PROGRESS in 1.0.5; candidate scale,
   ...
   budget and reviewed hybrid components exist. The next bounded task is a single
   mixed-region router/reflux/event-local probe. Scale/time/solver selection remains RED.
...
| Next stage? | Implement and compare an isolated hybrid/all-speed low-Mach plus event-local compressible policy candidate, then finish the PhysicalScale and mandatory solver/precision gates; production OmniAtmosphere remains unauthorized. |
