...
scaffold, shared contracts and Rusanov uniform/pressure-pulse/density-advection/contact-discontinuity/near-vacuum/Sod debug probes are now
...
| 3 | AtmosphereBench | 1.0.5 IN_PROGRESS: Rusanov has strict-double 1D uniform, pressure-pulse, density-advection, contact-discontinuity, near-vacuum and sealed Sod probes; HLLE/LBM remain registered-only |
...
| Next stage? | Add grid-refinement/low-Mach or leak Rusanov evidence while HLLE/LBM remain registered-only. Production OmniAtmosphere remains unauthorized. |
