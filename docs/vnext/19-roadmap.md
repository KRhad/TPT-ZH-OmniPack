...
| 3 | AtmosphereBench | 1.0.5 IN_PROGRESS: Rusanov is the debug floor, all-speed Rusanov is rejected, and HLLC/Rusanov-fallback is the unselected front-runner after current 1D low-Mach/vacuum/shock/leak probes plus periodic 2D uniform/pulse probes; sealed heating, convection, gas mixing, budget and physical-time policy remain open; HLLE/LBM remain registered-only |
...
| Next stage? | Continue HLLC/Rusanov-fallback only inside AtmosphereBench with sealed-heating/source-ledger, convection, gas-mixing, physical-time and budget gates; keep HLLE/LBM registered-only and production OmniAtmosphere unauthorized. |
