...
|---|---|---|---|---|---|
| C-01 | CRITICAL | correctness/performance | quantified at 7167 explicit HLLC substeps and 12121.47543 ms per 60 Hz candidate tick; direct real-acoustic mapping is rejected, while budget-limited uniform scaling would cap signals near 0.096 m/s and is also rejected as the default. Uncoupled bulk-transport and whole-case HLLC components now pass, but no router, cross-route face/reflux coupling, event-local subcycling, dynamic acoustic halo or target-grid event-fraction result exists | implement and validate a single mixed-region hybrid probe with promotion/demotion, equal-and-opposite interface exchange, reflux closure, hysteresis/conflicts, dynamic domain of dependence and worst-case cost | GREEN component sub-gate, RED coupled policy |
