...
|---|---|---|---|---|---|
| C-01 | CRITICAL | correctness/performance | quantified at 7167 explicit HLLC substeps and 12121.47543 ms per 60 Hz candidate tick; direct real-acoustic mapping is rejected, while budget-limited uniform scaling would cap signals near 0.096 m/s and is also rejected as the default. The 1D mixed-region probe now passes promotion/demotion, cross-route interface reflux, event-local substeps, hysteresis/threshold scan and global conservation, but reaches a full-domain event fraction of 1.0 and has no 2D/physical domain or target-grid budget result | extend the proof to 2D/domain-of-dependence, near-vacuum/species routing and target-grid worst-case cost; only then evaluate solver selection | GREEN 1D sub-gate, RED 2D/physical policy |
