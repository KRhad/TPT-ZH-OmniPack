...
|---|---|---|---|---|---|
| C-01 | CRITICAL | correctness/performance | direct real-acoustic mapping remains rejected at 7167 substeps/12121.47543 ms per 60 Hz tick; bounded 1D/2D ledgers, near-vacuum route and passive species interface fixtures pass, but 32x24 reaches event fraction 0.927083, physical acoustic domain is 1434 cells, and the three-grid matrix exceeds 4.16667 ms budget | choose a physical scale/time and general low-Mach pressure policy that meets domain/budget constraints; add evolving vacuum/species cases; keep solver selection RED | GREEN bounded routing evidence, RED physical policy |
