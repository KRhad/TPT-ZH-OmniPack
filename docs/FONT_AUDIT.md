...
当前原生 12px 方案生成 `resources/font.bz2` SHA-256 `2E9C51961996EC8C7CDDE709DDF1895A2A759755138D2B57FCC89072C5F4BC03`，包含 14,723 个字形，其中官方 TPT 基线 12,790 个、Fusion 补充 1,933 个、Unifont 回退 0 个。固定测试逐行比较 `U+4E2D/U+6587/U+7B80/U+4F53/U+5DE5/U+4E1A` 的 BDF 单色矩阵与 TPT 解包矩阵，并验证 2,687 个嵌入语言与周期表必需字符、替换字形、容器边界、码点排序、宽度和 pack/unpack。118 个周期表中文名称所需字符全部存在且非空；周期表三批文案按需补字，本批新增“征”`U+5F81` 与“泼”`U+6CFC`。既有 Fusion 中文显示曾由用户确认；新增元素和周期表界面字形尚未完成人工桌面逐字可读性检查，自动结果不替代最终视觉门禁。
