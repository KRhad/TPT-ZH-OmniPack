...
当前原生 12px 方案生成 `resources/font.bz2` SHA-256 `B10BE53C1678B20FD85B61E4156D2A1A45824F2FB81863FA747289808487C835`，包含 14,744 个字形，其中官方 TPT 基线 12,790 个、Fusion 补充 1,954 个、Unifont 回退 0 个。固定测试逐行比较 `U+4E2D/U+6587/U+7B80/U+4F53/U+5DE5/U+4E1A` 的 BDF 单色矩阵与 TPT 解包矩阵，并验证 2,708 个嵌入语言与周期表必需字符、替换字形、容器边界、码点排序、宽度和 pack/unpack。118 个周期表中文名称所需字符全部存在且非空；补充平面元素名的四字节 UTF-8 路径、无替换字形及完整语言包离屏渲染均有断言。无机第二批说明新增 `抵`（U+62B5）和 `铵`（U+94F5），两者均直接取自固定 Fusion 原生 12px BDF；构建门禁最初按预期拒绝缺字旧产物，重建后字体验证和目录离屏渲染通过。既有 Fusion 中文显示曾由用户确认；本批两个新字形仍未完成人工桌面逐字可读性检查，自动结果不替代最终视觉门禁。
