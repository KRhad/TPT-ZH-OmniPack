...
当前原生 12px 方案生成 `resources/font.bz2` SHA-256 `E01DEB25F6C85515BCE8353C34228E7F738E0A7997084FC569E6CB3BA2C0C01B`，包含 14,736 个字形，其中官方 TPT 基线 12,790 个、Fusion 补充 1,946 个、Unifont 回退 0 个。固定测试逐行比较 `U+4E2D/U+6587/U+7B80/U+4F53/U+5DE5/U+4E1A` 的 BDF 单色矩阵与 TPT 解包矩阵，并验证 2,700 个嵌入语言与周期表必需字符、替换字形、容器边界、码点排序、宽度和 pack/unpack。118 个周期表中文名称所需字符全部存在且非空；碳族语言项首次把补充平面字符“𫓧”`U+2B4E7` 放入实际图鉴后，离屏探针发现并促成四字节 UTF-8 解码修复，现在同时断言补充平面往返、无替换字形及完整语言包渲染。既有 Fusion 中文显示曾由用户确认；锕系新增“诱”字、此前各批新增字形和周期表界面字形尚未完成人工桌面逐字可读性检查，自动结果不替代最终视觉门禁。
