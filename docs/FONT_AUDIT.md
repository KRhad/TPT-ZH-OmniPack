...
当前原生 12px 方案生成 `resources/font.bz2` SHA-256 `E8906FC1135DE7ABC8322ACB262D4C79865B7E687D63076A74A055B3588B6C2B`，包含 14,799 个字形，其中官方 TPT 基线 12,790 个、Fusion 补充 2,009 个、Unifont 回退 0 个。固定测试逐行比较 `U+4E2D/U+6587/U+7B80/U+4F53/U+5DE5/U+4E1A` 的 BDF 单色矩阵与 TPT 解包矩阵，并验证 2,763 个嵌入语言与周期表必需字符、替换字形、容器边界、码点排序、宽度和 pack/unpack。118 个周期表中文名称所需字符全部存在且非空；补充平面元素名的四字节 UTF-8 路径、无替换字形及完整语言包离屏渲染均有断言。代表性核素首批新增 `塔/氚/育`，有机首批新增 `丁/丙/尿/烃/烯/烷/甲/脂/芳/苯/酮/饱/香`，第二批再新增 `橡/沥/淀/糖/纤/胺/萄/葡/酯/韧/龙`；生态与说明清理再补入 `伤/农/壤/少/携/涤/燥/血/谱/雨/霉/霾` 等 Fusion 原生字形。重建后的来源解码、覆盖、pack/unpack 和引擎离屏渲染已通过。既有 Fusion 中文显示曾由用户确认；本批新增字形仍未完成人工桌面逐字可读性检查，自动结果不替代最终视觉门禁。
