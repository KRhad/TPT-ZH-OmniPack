...
当前原生 12px 方案生成 `resources/font.bz2` SHA-256 `D2CE410314DD99220625B9F18A73DE88B35CF563A802AD57CACC96B30530502E`，包含 14,742 个字形，其中官方 TPT 基线 12,790 个、Fusion 补充 1,952 个、Unifont 回退 0 个。固定测试逐行比较 `U+4E2D/U+6587/U+7B80/U+4F53/U+5DE5/U+4E1A` 的 BDF 单色矩阵与 TPT 解包矩阵，并验证 2,706 个嵌入语言与周期表必需字符、替换字形、容器边界、码点排序、宽度和 pack/unpack。118 个周期表中文名称所需字符全部存在且非空；补充平面元素名的四字节 UTF-8 路径、无替换字形及完整语言包离屏渲染均有断言。无机首批 16 个中英文名称与说明没有引入新的缺字，字体文件及 SHA-256 保持不变。既有 Fusion 中文显示曾由用户确认；本批新增材料说明与此前新增稀有字形尚未完成人工桌面逐字可读性检查，自动结果不替代最终视觉门禁。
