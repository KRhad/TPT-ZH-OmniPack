...
当前原生 12px 方案生成 `resources/font.bz2` SHA-256 `49DFFFD38A3559DA115628DF52E7F1D8D01BC2622D5521D648F9D193E0A7EF48`，包含 14,713 个字形，其中官方 TPT 基线 12,790 个、Fusion 补充 1,923 个、Unifont 回退 0 个。固定测试逐行比较 `U+4E2D/U+6587/U+7B80/U+4F53/U+5DE5/U+4E1A` 的 BDF 单色矩阵与 TPT 解包矩阵，并验证 2,677 个嵌入语言与周期表必需字符、替换字形、容器边界、码点排序、宽度和 pack/unpack。118 个周期表中文名称所需字符全部存在且非空，其中 90 个是本次新增覆盖。既有 Fusion 中文显示曾由用户确认；新增稀有元素字形尚未完成人工桌面逐字可读性检查，自动结果不替代最终视觉门禁。
