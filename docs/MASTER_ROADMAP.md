...
| `0.2.0` | 四模块用途审计、四条跨模块闭环、7 个示例存档、8 项教程/挑战 | 48/48 用途矩阵与 fail-closed 审计已完成；四条闭环、示例和教程尚未实现 | `GATE-020-*` 全部通过并创建 `v0.2.0` |
...
48 元素用途矩阵与审计门禁已经完成；下一步先登记最小跨模块反应，不以元素数量为目标。四条必需链是冶金—化学、生态—化学、冶金—核工业和全模块废物处理；每条都必须有正常、误操作、事故、停止和回收路径及对应存档、说明、自动测试和压力测试。

### 0.3.0：工程自动化

优先组合或配置官方电子元件。只有组合无法满足且登记审计通过时才新增元素。传感器扫描必须有半径，事件必须有预算，信号必须有循环保护；只读与模块门禁对自动化同样生效。
