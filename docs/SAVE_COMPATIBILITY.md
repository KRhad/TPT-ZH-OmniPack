...
- 官方、工业冶金、局部生态、高级化学、受控核工业、完整周期表和混合场景的隔离 stamp 双往返已由当前超重元素 clean EXE 复跑；六类合计 204 粒子、每次加载 254 个字段断言和 203 个稳定/调色板 identifier；
...
- 包含全部 335 个活动材料的大型 OPS、多次反复加载和缺失新元素提示；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对未来扩展 ID 的端到端行为。
- 大型 335 材料同图反复加载、缺失 identifier 交互和外部网络保存路径；完整 118 周期元素同图 OPS 已实测通过。
