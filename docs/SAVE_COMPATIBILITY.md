...
- 当前容量：`PT_NUM=1024`、`PMAPBITS=10`；当前已登记的 `0..669` 数字槽与全部既有 identifier 均未重编号，其中 `642..669` 是预留槽。
...
- 无机三批固定为 `462..511`；工程材料区固定为 `512..575`，当前已实现 `OMNI_PT_SOLD..OMNI_PT_RFBK=512..532`；代表性核素固定为 `OMNI_PT_H2IS..OMNI_PT_CF52=576..588`；两批有机内容固定为 `OMNI_PT_CH4M..OMNI_PT_EACT=589..621`；电子与特殊材料首批固定为 `OMNI_PT_GAAS..OMNI_PT_DIEL=622..641`，`642..669` 预留，未来内容从 `670` 开始。
...
`high-id-save-probe` 覆盖直接高位材料、`LAVA.ctype`、`SPRK.ctype`、`CONV.ctype/tmp`、`VIRS.tmp2`、带来源标记的 `BRMT.ctype/tmp4` 和 identifier palette，并确认来源 `pmapbits=11` 的已知 `OMNI_PT_SOLD` 槽 1536 会映射到当前稳定 ID 512；缺失高位 identifier 会进入缺失报告且对应直接类型被中和，损坏 `pmapbits=0/17` 被拒绝。电子材料另以真实客户端三进程测试覆盖全部 `622..641` 直接类型及最高 ID `DIEL=641` 的间接携带字段。
...
- 官方、工业冶金、局部生态、化学/无机/有机、受控核工业/核素、完整周期表、电子与特殊材料七类隔离 stamp 双往返：21 个进程、346 粒子、每次加载合计 406 个字段断言、351 个稳定 identifier 和 346 个 palette identifier；电子类为 26 粒子、34 个字段断言，覆盖全部 20 个新材料及最高 `DIEL=641` 的 `LAVA/SPRK/CONV/VIRS` 携带字段；
...
- 五模块禁用回归保留 22 个粒子，包括 `NITI=520`、`RFBK=532`、`CF52=588`、`EACT=621`、`DIEL=641`、两粒 `ERES`、`CATA`、带标记 `BRMT(ctype=ALUM)`、生态、化学和核工业代表项；一模拟刻后既有化学反应保持暂停，已充电介电陶瓷也不会向邻近导体放电，Omni 更新事件为 0；
...
- 包含全部 471 个可玩材料的大型 OPS、多次反复加载和缺失高位 identifier 提示；兼容别名不作为直接样本凑数；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对高位 ID 的端到端行为；
