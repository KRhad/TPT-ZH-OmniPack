...
- 官方、工业冶金、局部生态、高级化学、受控核工业、周期表前八批和混合场景的隔离 stamp 双往返已由当前卤素 clean EXE 复跑；六类合计 123 粒子、每次加载 172 个字段断言；
- 周期表前八批 38 个新元素完成隔离 stamp 双往返：3 个进程、2 次重启、2 次加载、44 粒子、每次加载 52 个字段断言、38 个大于 255 的直接类型；
...
- `370..461` 全批元素实现后的完整 118 元素同图 OPS、大型反复加载和缺失 identifier 交互；当前前八批 38 个新 ID 已实测通过。
