...
- 当前容量：`PT_NUM=1024`、`PMAPBITS=10`；当前已登记的 `0..685` 数字槽与全部既有 identifier 均未重编号，其中 `642..669` 是预留槽。
...
- 无机三批固定为 `462..511`；工程材料区固定为 `512..575`，当前已实现 `OMNI_PT_SOLD..OMNI_PT_RFBK=512..532`；代表性核素固定为 `OMNI_PT_H2IS..OMNI_PT_CF52=576..588`；两批有机内容固定为 `OMNI_PT_CH4M..OMNI_PT_EACT=589..621`；电子与特殊材料首批固定为 `OMNI_PT_GAAS..OMNI_PT_DIEL=622..641`，`642..669` 预留；生态与污染首批固定为 `OMNI_PT_SOIL..OMNI_PT_DETG=670..685`，未来内容从 `686` 开始。
...
`high-id-save-probe` 覆盖直接高位材料、`LAVA.ctype`、`SPRK.ctype`、`CONV.ctype/tmp`、`VIRS.tmp2`、带来源标记的 `BRMT.ctype/tmp4` 和 identifier palette，并确认来源 `pmapbits=11` 的已知 `OMNI_PT_SOLD` 槽 1536 会映射到当前稳定 ID 512；缺失高位 identifier 会进入缺失报告且对应直接类型被中和，损坏 `pmapbits=0/17` 被拒绝。电子与环境材料另以真实客户端三进程测试覆盖全部 `622..641`、`670..685` 直接类型及最高 ID `DETG=685` 的间接携带字段。
...
- 官方、工业冶金、局部生态、化学/无机/有机、受控核工业/核素、完整周期表、电子与特殊材料、生态污染扩展八类隔离 stamp 双往返：24 个进程、368 粒子、每次加载合计 436 个字段断言、374 个稳定 identifier 和 368 个 palette identifier；环境类为 22 粒子、30 个字段断言，覆盖全部 16 个新材料及最高 `DETG=685` 的 `LAVA/SPRK/CONV/VIRS` 携带字段；
- mixed OPS：3 个进程、11 粒子、21 个字段断言；
- 化学隔离 stamp 覆盖 93 个化学/无机/有机元素；回归为 99 粒子、107 个字段断言，并以最高 ID `EACT=621` 覆盖 `LAVA/SPRK/CONV/VIRS` 间接类型字段；
- 全部 118 个周期元素在同一隔离 stamp 中完成双往返，直接类型和 `LAVA/SPRK/BRMT/CONV/VIRS` 携带字段均覆盖；
- 五模块禁用回归保留 25 个粒子，包括 `NITI=520`、`RFBK=532`、`CF52=588`、`EACT=621`、`DIEL=641`、`DETG=685`、两粒 `ERES`、`CATA`、`DETG+OIL+WATR`、带标记 `BRMT(ctype=ALUM)`、生态、化学和核工业代表项；一模拟刻后既有化学和环境反应保持暂停，已充电介电陶瓷也不会向邻近导体放电，Omni 更新事件为 0；
...
- 包含全部 466 个菜单材料并覆盖 487 个活动非别名类型的大型 OPS、多次反复加载和缺失高位 identifier 提示；兼容别名不作为直接样本凑数；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对高位 ID 的端到端行为；
