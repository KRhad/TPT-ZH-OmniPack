...
- 周期表首批 7 个新元素（ID `370/375/379/390/405/431/461`）完成隔离 stamp 双往返：3 个进程、2 次重启、2 次加载、13 粒子、每次加载 21 个字段断言；
...
- `370..461` 全批元素实现后的完整 118 元素同图 OPS、大型反复加载和缺失 identifier 交互；首批 7 个新 ID 已实测通过。
