...
- 当前 `462..511` 高位连续区已占满。扩大到 10 位会使 `can_move` 矩阵由 512² 增为 1024²，并触及 OPS、Lua、菜单及所有打包类型字段；下一批材料必须先完成独立扩容审计，在此之前禁止分配 `512+`。
...
- 官方、工业冶金、局部生态、化学与无机物、受控核工业、完整周期表和混合场景的隔离 stamp 双往返已由元素去重 clean EXE 复跑；六类合计 253 粒子、每次加载 305 个字段断言、257 个稳定 identifier 和 253 个 palette identifier；
- 化学隔离 stamp 覆盖全部 60 个化学/无机元素：3 个进程、2 次重启、2 次加载、66 粒子、每次加载 74 个字段断言和 66 个稳定/调色板 identifier；最高 ID `AMCL=511` 及第三批类型同时进入 `LAVA/SPRK/CONV/VIRS` 携带字段路径；
- 四模块禁用回归使用启用进程创建 OPS，再由全部模块关闭的新进程载入：冶金带标记 `BRMT(ctype=ALUM)`、生物 `STER+PATH`、化学 `HCLA+NAOH+CARA+AMCL`、核工业 `NCLT+NWST` 共 9 粒子全部保留，一模拟刻后类型、`ctype/tmp4` 与其他载体字段不变且 Omni 事件为 0；
- 全部 118 个周期元素在同一隔离 stamp 中完成双往返：3 个进程、2 次重启、2 次加载、125 粒子、每次加载 135 个字段断言、118 个直接周期类型，其中 103 个 ID 大于 255；
- 周期类型在 `LAVA.ctype`、`SPRK.ctype`、`CONV.ctype/tmp`、`VIRS.tmp2` 中的 identifier palette 和 9 位携带值均保持；
- 直接 ID 大于 255 和 `LAVA/SPRK/BRMT/CONV/VIRS` 间接类型字段已有覆盖；可回收 `BRMT` 同时断言专用 `tmp4` 标记；
- 旧 `MSCR=278` 另有独立三进程迁移回归：第一进程通过旧载体内部生成并保存真实 278 粒子，第二进程加载后首刻迁移为 `BRMT=30` 并保留来源 `ctype`，第三进程确认新 OPS 不含 278 粒子且 `ctype/tmp4` 不变；直接放置和菜单选择均被阻止；
...
- 包含全部 384 个可玩材料的大型 OPS、多次反复加载和缺失新元素提示；兼容别名不作为直接样本凑数；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对未来扩展 ID 的端到端行为。
- 大型 384 可玩材料同图反复加载、缺失 identifier 交互和外部网络保存路径；完整 118 周期元素同图 OPS 已实测通过。
