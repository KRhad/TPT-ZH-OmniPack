...
- 无机三批固定为 `462..511`；工程材料区固定为 `512..575`，当前已实现 `OMNI_PT_SOLD..OMNI_PT_RFBK=512..532`；未来内容区为 `576..1023`。
...
`high-id-save-probe` 已对直接 `SOLD=512/RFBK=532`、`LAVA.ctype`、`SPRK.ctype`、`BRMT.ctype/tmp4`、`CONV.ctype/tmp`、`VIRS.tmp2` 和 identifier palette 执行原生序列化往返，并确认来源 `pmapbits=11` 的已知 `OMNI_PT_SOLD` 槽 1536 会映射到当前稳定 ID 512；缺失高位 identifier 会进入缺失报告且对应直接类型被中和，损坏 `pmapbits=0/17` 被拒绝。
...
- 官方、工业冶金、局部生态、化学与无机物、受控核工业、完整周期表六类隔离 stamp 双往返：18 个进程、274 粒子、每次加载合计 326 个字段断言、278 个稳定 identifier 和 274 个 palette identifier；冶金样本覆盖全部 43 个可玩材料及最高 `OMNI_PT_RFBK=532`；
...
- 四模块禁用回归保留 11 个粒子，包括 `NITI=520`、`RFBK=532`、带标记 `BRMT(ctype=ALUM)`、生态、化学和核工业代表项；一模拟刻后 Omni 更新事件为 0；
...
- 包含全部 405 个可玩材料的大型 OPS、多次反复加载和缺失高位 identifier 提示；兼容别名不作为直接样本凑数；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对高位 ID 的端到端行为；
