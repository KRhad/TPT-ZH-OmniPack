...
- 官方、工业冶金、局部生态、化学与无机物、受控核工业、完整周期表和混合场景的隔离 stamp 双往返已由最终无机第三批 clean EXE 复跑；六类合计 254 粒子、每次加载 304 个字段断言和 253 个稳定/调色板 identifier；
...
- 四模块禁用回归使用启用进程创建 OPS，再由全部模块关闭的新进程载入：冶金 `MSCR`、生物 `STER+PATH`、化学 `HCLA+NAOH+CARA+AMCL`、核工业 `NCLT+NWST` 共 9 粒子全部保留，一模拟刻后类型/载体字段不变且 Omni 事件为 0；
- 全部 118 个周期元素在同一隔离 stamp 中完成双往返：3 个进程、2 次重启、2 次加载、125 粒子、每次加载 134 个字段断言、118 个直接周期类型，其中 103 个 ID 大于 255；
...
- 包含全部 385 个活动材料的大型 OPS、多次反复加载和缺失新元素提示；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对未来扩展 ID 的端到端行为。
- 大型 385 材料同图反复加载、缺失 identifier 交互和外部网络保存路径；完整 118 周期元素同图 OPS 已实测通过。
