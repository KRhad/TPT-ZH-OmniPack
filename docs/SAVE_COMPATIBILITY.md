...
- 当前容量：`PT_NUM=1024`、`PMAPBITS=10`；当前已登记的 `0..621` 数字 ID 与全部 identifier 均未重编号。
...
- 无机三批固定为 `462..511`；工程材料区固定为 `512..575`，当前已实现 `OMNI_PT_SOLD..OMNI_PT_RFBK=512..532`；代表性核素固定为 `OMNI_PT_H2IS..OMNI_PT_CF52=576..588`；两批有机内容固定为 `OMNI_PT_CH4M..OMNI_PT_EACT=589..621`；未来内容区为 `622..1023`。
...
`high-id-save-probe` 已对直接 `SOLD=512/RFBK=532/CF52=588/EACT=621`、以 621 为目标的 `LAVA.ctype`、`SPRK.ctype`、`CONV.ctype/tmp`、`VIRS.tmp2`、带 `NITI` 来源的 `BRMT.ctype/tmp4` 和 identifier palette 执行原生序列化往返，并确认来源 `pmapbits=11` 的已知 `OMNI_PT_SOLD` 槽 1536 会映射到当前稳定 ID 512；缺失高位 identifier 会进入缺失报告且对应直接类型被中和，损坏 `pmapbits=0/17` 被拒绝。
...
- 官方、工业冶金、局部生态、化学/无机/有机、受控核工业/核素、完整周期表六类隔离 stamp 双往返：18 个进程、320 粒子、每次加载合计 372 个字段断言、324 个稳定 identifier 和 320 个 palette identifier；核类样本覆盖 7 个核工业材料及全部 13 个核素，化学类覆盖 60 个既有材料及两批 33 个有机元素，最高为 `OMNI_PT_EACT=621`；
...
- 四模块禁用回归保留 20 个粒子，包括 `NITI=520`、`RFBK=532`、`CF52=588`、`EACT=621`、两粒 `ERES`、`CATA`、带标记 `BRMT(ctype=ALUM)`、生态、化学和核工业代表项；一模拟刻后 `FATS+CAUS` 与 `2×ERES+CATA` 仍保持原类型且 Omni 更新事件为 0；
...
- 包含全部 451 个可玩材料的大型 OPS、多次反复加载和缺失高位 identifier 提示；兼容别名不作为直接样本凑数；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对高位 ID 的端到端行为；
