...
- 官方、工业冶金、局部生态、化学与无机物、受控核工业、完整周期表和混合场景的隔离 stamp 双往返已由最终无机首批 clean EXE 复跑；六类合计 220 粒子、每次加载 270 个字段断言和 219 个稳定/调色板 identifier；
- 化学隔离 stamp 覆盖全部 26 个化学/无机元素：3 个进程、2 次重启、2 次加载、32 粒子、每次加载 40 个字段断言和 32 个稳定/调色板 identifier；高 ID 无机类型同时进入 `LAVA/SPRK/CONV/VIRS` 携带字段路径；
- 四模块禁用回归使用启用进程创建 OPS，再由全部模块关闭的新进程载入：冶金 `MSCR`、生物 `STER+PATH`、化学 `HCLA+NAOH`、核工业 `NCLT+NWST` 共 7 粒子全部保留，一模拟刻后类型/载体字段不变且 Omni 事件为 0；
...
- 包含全部 351 个活动材料的大型 OPS、多次反复加载和缺失新元素提示；
- 外部旧模组存档的合法来源识别和逐项迁移；
- 网络保存服务对未来扩展 ID 的端到端行为。
- 大型 351 材料同图反复加载、缺失 identifier 交互和外部网络保存路径；完整 118 周期元素同图 OPS 已实测通过。
