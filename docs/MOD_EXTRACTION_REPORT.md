...
total_omnipack_elements=211
compatibility_aliases=1
total_omnipack_playable=210
...
当前新增 92 个周期元素、无机三批 50 个材料、工程合金 `SOLD..NITI=512..520` 及矿物/陶瓷/玻璃首批 `GYPS..RFBK=521..532` 均为 OmniPack 原创族/反应逻辑，不计入第三方 `elements_ported` 或 `elements_rewritten`；外部候选只用于确认搜索覆盖，没有复制其实现。`first_port_batch_complete=false` 仍是许可证门禁的真实结果，不能把原创内容批次冒充成已完成第三方移植。
