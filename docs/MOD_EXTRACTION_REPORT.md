...
当前新增 83 个周期元素是 OmniPack 原创族逻辑，不计入第三方 `elements_ported` 或 `elements_rewritten`；外部候选只用于确认搜索覆盖，没有复制其实现。
