...
| 周期表前十三批 83 | PASS | PASS | PASS | 预算帧 PASS | 新 ID `370..461` 中 83 项启用；109/118 映射可用 |
...
| 83 个新 identifier / 固定 ID / 双语图鉴 | PASS | 元素登记、i18n、内容门禁；所有新增说明均以对应元素中英文名开头 |
...
| 锕系关键行为 | PASS | 15 个成员、分级寿命与物性、代表性后代、固态/熔融衰变、八段中子嬗变、锎有限裂变代理、铹终端吸收、酸氧与汽化均由真实客户端断言 |
| 单帧事件预算 | PASS | 1,200 个衰变或反应隔离样本逐批验证；锕系 1,200 粒子样本峰值不超过且可达到 `1024` |
| 周期 ID OPS 与携带字段 | PASS | 3 进程、2 重启、2 加载、92 粒子、101 字段断言、83 个大于 255 的直接类型，覆盖 `MSCR.ctype` |
| 周期内容无解锁直接选择 | PASS | 从 `HE`、`NA`、`CA` 到 `HF`、`LA`、`AC` 的各批代表项均可直接选择 |
| 新增中文字体与 Unicode 路径 | PASS | 字体为 14,736 字形/2,700 必需字符；`𫓧` 的四字节 UTF-8 往返、字体查找和完整中文语言包离屏渲染通过；新增字形人工视觉 NOT RUN |
| 本批 clean Release build | PASS | `build-periodic-actinide-final-clean`，`596/596`；最终 EXE `ADB2C2C9AFE6D4C6267F153D6EBDF6967CBE9FC981CBE31F4EDE2A5EAFAFBEC0` |
| 本批 Meson/Python全量套件 | PASS | static `21/21`；Python `158/158`，0 skip |
| 六类与 mixed OPS | PASS | 21 个 OPS 进程、14 重启、14 加载；六类 171 粒子/221 字段断言和 170 个稳定/调色板 identifier，mixed 11 粒子/20 字段断言 |
