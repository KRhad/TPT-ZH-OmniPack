...
| 测试 | 类型 | 状态 | 证据/备注 |
|---|---|---|---|
| 11 类自动化能力设计 | 源码/自动 | PASS | 全部复用官方元件；17 个官方源码指纹；新增元素 0；ID `392..423` 保留 |
| 元素说明自识别与双源一致 | 自动 | PASS | 48/48 英文 `Name: ...`、中文 `名称：……`；登记表/语言包/名称键漂移 fail-closed |
| clean Release build | 构建 | PASS | `build-0.3.0-dev-final-clean`，`502/502`；剥离 EXE 与压力候选哈希一致 |
| 开发 HEAD 自动测试 | 自动 | PASS | Meson static `16/16`；Python `121/121`、0 skip；i18n errors 0 |
| 自动化场景 / 挑战 | 实际运行 | PASS | 候选 EXE `FBCA1387...0FF2`；9/9 OPS、6/6 挑战、95 断言、停止增量 0 |
| 两代内容绑定 | 实际运行 | PASS | 同一 EXE：0.2 示例 7/7、教程 8/8；0.3 场景 9/9、挑战 6/6 |
| 0.3 本地包 | 包 | PASS | 普通 `BB57D87F...07EA`、符号 `7AAD0AB5...ACAE`；成员、PE、路径、哈希与解压二审通过 |
| S11 自动工厂正式压力 | 持续运行 | PASS | `20260731T080609Z-91b43bef`，`60.001+600.005` 秒；事件 5233、官方信号 74380132、`gate=true` |
| S12 一次性火花样本 | 持续运行 | FAIL | `20260731T081741Z-ddd919fe`；预热后信号 0，`signal_behavior_pass=false`、`gate=false`，保留为失败证据 |
| S12 持续信号正式压力 | 持续运行 | PASS | `20260731T083137Z-80a982fe`；官方 BTRY 激励，`60.016+600.017` 秒、信号 978876949、峰值 56421、`gate=true` |
...
| 自动化玩法 | PASS（本地自动/运行；可信 GUI 未测试） |
| 灾害玩法 | NOT RUN |
| 炼金进度、成就 | NOT RUN |
| 0.2/0.3 挑战、教程 | PASS（本地运行） |
| 固定高粒子数压力样本 | PASS（0.1 十项、0.2 四项、0.3 两项） |
| FPS、最低 FPS、内存和有限增长记录 | PASS（现有固定样本；不等于长期证明） |
