...
| 周期表前十二批 70 | PASS | PASS | PASS | 预算帧 PASS | 新 ID `370..461` 中 70 项启用；96/118 映射可用 |
...
| 70 个新 identifier / 固定 ID / 双语图鉴 | PASS | 元素登记、i18n、内容门禁；所有新增说明均以对应元素中英文名开头 |
...
| 单帧事件预算 | PASS | 1,200 个衰变或反应隔离样本逐批验证；镧系 1,200 粒子样本峰值不超过且可达到 `1024` |
| 周期 ID OPS 与携带字段 | PASS | 3 进程、2 重启、2 加载、76 粒子、84 字段断言、70 个大于 255 的直接类型 |
| 周期内容无解锁直接选择 | PASS | 从 `HE`、`NA`、`CA` 到 `HF`、`LA` 的各批代表项均可直接选择 |
| 新增中文字体与 Unicode 路径 | PASS | 字体为 14,735 字形/2,699 必需字符；`𫓧` 的四字节 UTF-8 往返、字体查找和完整中文语言包离屏渲染通过；新增字形人工视觉 NOT RUN |
| 本批 clean Release build | PASS | `build-periodic-lanthanide-final-clean`，`583/583`；最终 EXE `DD32638A554B00EF918D921C155126594E502D98F15A99E6597E326922BCF3E8` |
| 本批 Meson/Python 全量套件 | PASS | static `21/21`；Python `157/157`，0 skip |
| 六类与 mixed OPS | PASS | 21 个 OPS 进程、14 重启、14 加载；六类 155 粒子/204 字段断言和 154 个稳定/调色板 identifier，mixed 11 粒子/20 字段断言 |
