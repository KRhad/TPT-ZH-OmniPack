...
| 周期表按钮静态避让 | PASS | `P` 与搜索按钮统一固定在最右竖列 `WINDOWW-16`，分别位于 `WINDOWH-48/-32`；分类按钮从 `WINDOWH-64` 起排，30 px 元素按钮滚动区按完整步长结束于右栏之前 |
| 周期表按钮实际视觉 | PASS | 96 DPI、游戏 2× 缩放下，实屏显示 `P` 与右栏对齐；鼠标点击其中心后打开 118 格周期表。证据：`artifacts/gui/0.7.0-dev/20260802T021500Z-right-column/`，截图 SHA-256 `70458914...E248`、`3EF4E394...1A2C`；不替代 125%/150% DPI 矩阵 |
...
| 周期表 `P` 按钮 96 DPI / 游戏 2× 鼠标点击 | PASS |
