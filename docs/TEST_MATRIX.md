...
| 周期表前八批 38 | PASS | PASS | PASS | 预算帧 PASS | 新 ID `370..461` 中 38 项启用；64/118 映射可用 |
...
| 38 个新 identifier / 固定 ID / 双语图鉴 | PASS | 元素登记、i18n、内容门禁；所有新增说明均以对应元素中英文名开头 |
...
| 单帧事件预算 | PASS | 1,200 个 `OG/FR/RA/NH/FL/MC/LV/TS(tmp=1)` 隔离样本分别验证，峰值均不超过且可达到 `1024` |
| 周期 ID OPS 与携带字段 | PASS | 3 进程、2 重启、2 加载、44 粒子、52 字段断言 |
| 周期内容无解锁直接选择 | PASS | `HE`、`NA`、`CA`、`B`、`GE`、`FL`、`N`、`MC`、`S`、`LV`、`F`、`BR`、`TS` 代表项均可直接选择 |
| 新增中文字体与 Unicode 路径 | PASS | 字体为 14,732 字形/2,696 必需字符；`𫓧` 的四字节 UTF-8 往返、字体查找和完整中文语言包离屏渲染通过 |
| 本批 clean Release build | PASS | `build-periodic-halogen-final-clean`，`551/551`；最终 EXE `2D70ADE599F15718A415B813E4D918C62C3C641852F6AA6CBF0F05321C868F08` |
| 本批 Meson/Python 全量套件 | PASS | static `21/21`；Python `153/153`，0 skip |
| 六类与 mixed OPS | PASS | 21 进程、14 重启、14 加载；六类 123 粒子/172 字段断言，mixed 11 粒子/20 字段断言 |
