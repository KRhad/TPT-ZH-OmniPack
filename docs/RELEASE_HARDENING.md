...
py -3.14 tools/package_test_release.py --executable artifacts/release-hardening/tpt-zh-omnipack.exe --symbols artifacts/release-hardening/tpt-zh-omnipack.debug --objdump C:\\msys64\\ucrt64\\bin\\objdump.exe --strings C:\\msys64\\ucrt64\\bin\\strings.exe
```

`release + debug=true` produces a RelWithDebInfo-equivalent input. `objcopy --only-keep-debug` runs before `strip --strip-debug`; the normal ZIP never receives the detached symbols. `prepare_windows_release.py` passes one deterministic `SOURCE_DATE_EPOCH` to both GNU tools (the caller value when present, otherwise the PE/ZIP-safe `1980-01-01` default), so repeated separation of the same raw executable does not acquire the current clock time. `-Dstatic=prebuilt` alone is insufficient: the documented GCC runtime link flags prevent an otherwise clean-machine failure on `libgcc_s_seh-1.dll`, `libstdc++-6.dll` or `libwinpthread-1.dll`. The audit fails if these imports, user path markers, a `.debug*` PE section, or required ASLR/DEP/high-entropy flags remain absent.
