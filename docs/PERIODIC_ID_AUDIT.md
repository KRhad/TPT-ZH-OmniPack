...
periodic_elements_implemented=64
periodic_elements_planned=54
periodic_new_elements_active=38
active_element_slots=281
...
`370..461` 已全部显式登记为周期表固定区；当前启用 `HE=370`、`BE=371`、`B=372`、`N=373`、`F=374`、`NE=375`、`NA=376`、`P=377`、`S=378`、`AR=379`、`K=380`、`CA=381`、`GA=385`、`GE=386`、`AS=387`、`SE=388`、`BR=389`、`KR=390`、`SR=391`、`IN=401`、`SB=402`、`TE=403`、`I=404`、`XE=405`、`CS=406`、`BA=407`、`TL=428`、`BI=429`、`AT=430`、`RN=431`、`FR=432`、`RA=433`、`NH=456`、`FL=457`、`MC=458`、`LV=459`、`TS=460`、`OG=461`，其余 54 槽继续是不可选择的保留项。既有 `CHLR=360`、`MAGN=261`、`ALUM=256`、`TIN=259`、`LEAD=258` 和官方 `DMND=28`、`SLCN=187`、`O2=61`、`POLO=182` 继续复用且 ID 不变。自动化回归仍只复用官方元素，不拥有新稳定 ID。
...
完成全部 92 个新增周期元素后，活动元素预计为 `335`。当前活动数为 `281`（原 243 加前八批 38）；`462..511` 仍有 50 个连续槽，现有冶金、生物和核工业区还分别有 9、32 和 25 个空槽。理论活动上限仍可达到 451，覆盖 300–450 的目标上界而无需把 `PMAPBITS` 扩大到 10。

## 仍需实测

- 前八批 38 个新周期直接类型和周期类型在 `LAVA/SPRK/CONV/VIRS` 携带字段中的 OPS 双往返已通过；后续批次仍须逐批复跑；
