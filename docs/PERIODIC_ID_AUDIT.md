...
organic_content_ids=589..621
future_content_ids=622..1023
engine_active_element_slots=452
compatibility_aliases=1
playable_materials=451
registered_element_slots=622
...
- 直接 `SOLD=512/RFBK=532/CF52=588/EACT=621`；
- 621 号 `LAVA.ctype`、`SPRK.ctype`、`CONV.ctype/tmp`、`VIRS.tmp2`，以及 `BRMT.ctype/tmp4` 的 520 号可回收来源；
- palette 中的 `OMNI_PT_SOLD` 与 `OMNI_PT_EACT`；
...
`PT_NUM=1024` 使原生 `can_move` 和 Lua `customCanMove` 各包含 `1024²` 个单字节条目，即各 1,048,576 字节。当前登记到 621：共有 452 个活动项，其中 `MSCR=278` 是兼容别名，实际可玩材料为 451；显式保留槽 170 个，`622..1023` 仍有 402 个尚未登记的容量槽。

当前已登记 ID `0..621`、identifier 与模块所有权全部保持。兼容别名 278 永久保留，不因行为合并而释放。继续新增内容仍需逐批登记、预算和 OPS 测试，但本轮 10 位容量决策已经完成，不再以“禁止分配 512+”阻塞后续内容批次。

## 仍需实测

- 缺失高位 identifier 的实际 GUI 提示与外部网络保存服务路径；
- GUI `.cps` 保存、覆盖和取消的完整点击路径；
- 包含全部 451 个可玩材料的大型 OPS 反复加载；
