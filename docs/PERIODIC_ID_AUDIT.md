...
organic_content_ids=589..601
future_content_ids=602..1023
engine_active_element_slots=432
compatibility_aliases=1
playable_materials=431
registered_element_slots=602
...
- 直接 `SOLD=512/RFBK=532/CF52=588/FATS=601`；
- 601 号 `LAVA.ctype`、`SPRK.ctype`、`CONV.ctype/tmp`、`VIRS.tmp2`，以及 `BRMT.ctype/tmp4` 的 520 号可回收来源；
- palette 中的 `OMNI_PT_SOLD` 与 `OMNI_PT_FATS`；
...
`PT_NUM=1024` 使原生 `can_move` 和 Lua `customCanMove` 各包含 `1024²` 个单字节条目，即各 1,048,576 字节。当前登记到 601：共有 432 个活动项，其中 `MSCR=278` 是兼容别名，实际可玩材料为 431；显式保留槽 170 个，`602..1023` 仍有 422 个尚未登记的容量槽。
