...
$autorunSource = Join-Path $scriptRoot "runtime\omni_chemistry_runtime.lua"
$tempParent = [System.IO.Path]::GetFullPath($TemporaryDirectory)
$testRoot = Join-Path $tempParent ("tpt-omni-chemistry-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $resultPath = Join-Path $resolvedTestRoot "omni-chemistry-runtime.result"
    ...
    do {
        ...
        if ($resultText -match "(?m)^OMNI_CHEMISTRY_STATUS=PASS\r?$") { $passed = $true; break }
        if ($resultText -match "(?m)^OMNI_CHEMISTRY_STATUS=FAIL\r?$") { break }
    } while ([DateTime]::UtcNow -lt $deadline -and -not $process.HasExited)
    if (-not $passed) { throw "OmniChemistry Lua runtime failed; result=$resultText; artifacts=$resolvedTestRoot" }
    Write-Output "runtime-lua-omni-chemistry: PASS"
