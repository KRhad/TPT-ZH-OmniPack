...
$autorunSource = Join-Path $scriptRoot "runtime\metallurgy_regression.lua"
...
$testRoot = Join-Path $tempParent (
    "tpt-omnipack-metallurgy-" + [guid]::NewGuid().ToString("N")
)
...
try {
    ...
    $result = Join-Path $resolvedTestRoot "lua-metallurgy-regression.result"
    ...
    do {
        ...
        if ($resultText -match "(?m)^OMNI_METALLURGY_STATUS=PASS\r?$") {
            $passed = $true
            break
        }
        if ($resultText -match "(?m)^OMNI_METALLURGY_STATUS=FAIL\r?$") {
            break
        }
    ...
    if (-not $passed) {
        $failureMessage = (
            "Lua metallurgy regression failed; responding=$responding; " +
    ...
    if (-not $responding) {
        $unresponsiveMessage = (
            "Lua metallurgy regression produced PASS but the client stopped " +
            "responding; artifacts=$resolvedTestRoot"
        )
        throw $unresponsiveMessage
    }

    Write-Output "runtime-lua-metallurgy-test: PASS"
