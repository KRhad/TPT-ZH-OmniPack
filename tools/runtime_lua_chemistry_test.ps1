...
$autorunSource = Join-Path $scriptRoot "runtime\chemistry_regression.lua"
...
$testRoot = Join-Path $tempParent ("tpt-omnipack-chemistry-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $result = Join-Path $resolvedTestRoot "lua-chemistry-regression.result"
    ...
    do {
        ...
        if ($resultText -match "(?m)^OMNI_CHEMISTRY_STATUS=FAIL\r?$") {
            break
        }
    ...
    if (-not $passed) {
        throw "Lua chemistry regression failed; responding=$responding; result=$resultText; artifacts=$resolvedTestRoot"
    }
    if (-not $responding) {
        throw "Lua chemistry regression produced PASS but the client stopped responding; artifacts=$resolvedTestRoot"
    }
    Write-Output "runtime-lua-chemistry-test: PASS"
