...
$autorunSource = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "runtime\omni_corrosion_runtime.lua"
$tempParent = [System.IO.Path]::GetFullPath($TemporaryDirectory)
$testRoot = Join-Path $tempParent ("tpt-omni-corrosion-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $resultPath = Join-Path $resolvedTestRoot "omni-corrosion-runtime.result"
    ...
    do {
        ...
        if ($resultText -match "(?m)^OMNI_CORROSION_STATUS=PASS\r?$") { $passed = $true; break }
        if ($resultText -match "(?m)^OMNI_CORROSION_STATUS=FAIL\r?$") { break }
    } while ([DateTime]::UtcNow -lt $deadline -and -not $process.HasExited)
    if (-not $passed) { throw "OmniCorrosion Lua runtime failed; result=$resultText; artifacts=$resolvedTestRoot" }
    Write-Output "runtime-lua-omni-corrosion: PASS"
