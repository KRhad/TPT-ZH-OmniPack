...
$autorunSource = Join-Path $scriptRoot "runtime\materials_regression.lua"
if (-not (Test-Path -LiteralPath $autorunSource -PathType Leaf)) {
    throw "Missing Lua materials regression script: $autorunSource"
}
...
$testRoot = Join-Path $tempParent (
    "tpt-omnipack-materials-" + [guid]::NewGuid().ToString("N")
)
...
try {
    ...
    $result = Join-Path $resolvedTestRoot "lua-materials-regression.result"
    ...
    do {
        ...
        if (Test-Path -LiteralPath $result -PathType Leaf) {
            ...
            if ($resultText -match "(?m)^OMNI_MATERIALS_STATUS=(PASS|FAIL)\r?$") {
                break
            }
        }
        $process.Refresh()
    ...
    if ($resultText -match "(?m)^OMNI_MATERIALS_STATUS=PASS\r?$") {
        $passed = $true
    }
    $responding = $false
    ...
    if (-not $passed) {
        throw (
            "Lua materials regression failed; responding=$responding; " +
            "result=$resultText; artifacts=$resolvedTestRoot"
        )
    }
    if (-not $responding) {
        throw "Lua materials regression passed but client is unresponsive"
    }
    Write-Output "runtime-lua-materials-test: PASS"
