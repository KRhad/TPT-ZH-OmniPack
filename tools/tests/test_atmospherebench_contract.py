...
class AtmosphereBenchSourceContractTests(unittest.TestCase):
    ...
    def test_low_mach_projection_target_matrix_is_strict_and_fail_closed(self) -> None:
        ...
        self.assertIn("MultigridVCycle", source)
        self.assertIn("RestrictResidual", source)
        self.assertIn("ProlongateAndAdd", source)
        self.assertIn("RunMultigridMedianSample<153, 96>", source)
        self.assertIn("RunMultigridMedianSample<306, 192>", source)
        self.assertIn("RunMultigridMedianSample<612, 384>", source)
        self.assertIn("selected_low_mach_component=geometric_multigrid_v_cycle", source)
