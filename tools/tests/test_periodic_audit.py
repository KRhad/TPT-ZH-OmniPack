...
class PeriodicAuditTests(unittest.TestCase):
    ...
    def test_missing_molten_third_transition_hook_is_rejected(self) -> None:
        lava_path = ROOT / "src" / "simulation" / "elements" / "LAVA.cpp"
        source = lava_path.read_text(encoding="utf-8")
        original = periodic_audit.read_text
        errors: list[str] = []
        try:
            periodic_audit.read_text = lambda path, output: (
                source.replace(
                    "OmniMoltenThirdTransitionUpdate", "removedThirdTransitionHook"
                )
                if path == lava_path else original(path, output)
            )
            periodic_audit.check_engine(ROOT, errors)
            ...
            periodic_audit.read_text = original
        self.assertTrue(any("molten third-transition" in error for error in errors))

    def test_missing_molten_lanthanide_hook_is_rejected(self) -> None:
        lava_path = ROOT / "src" / "simulation" / "elements" / "LAVA.cpp"
        source = lava_path.read_text(encoding="utf-8")
        original = periodic_audit.read_text
        errors: list[str] = []
        try:
            periodic_audit.read_text = lambda path, output: (
                source.replace(
                    "OmniMoltenLanthanideUpdate", "removedLanthanideHook"
                )
                if path == lava_path else original(path, output)
            )
            periodic_audit.check_engine(ROOT, errors)
            ...
            periodic_audit.read_text = original
        self.assertTrue(any("molten lanthanide" in error for error in errors))

    def test_missing_molten_actinide_hook_is_rejected(self) -> None:
        lava_path = ROOT / "src" / "simulation" / "elements" / "LAVA.cpp"
        source = lava_path.read_text(encoding="utf-8")
        original = periodic_audit.read_text
        errors: list[str] = []
        try:
            periodic_audit.read_text = lambda path, output: (
                source.replace(
                    "OmniMoltenActinideUpdate", "removedActinideHook"
                )
                if path == lava_path else original(path, output)
            )
            periodic_audit.check_engine(ROOT, errors)
            ...
            periodic_audit.read_text = original
        self.assertTrue(any("molten actinide" in error for error in errors))

    def test_missing_molten_superheavy_hook_is_rejected(self) -> None:
        lava_path = ROOT / "src" / "simulation" / "elements" / "LAVA.cpp"
        source = lava_path.read_text(encoding="utf-8")
        original = periodic_audit.read_text
        errors: list[str] = []
        try:
            periodic_audit.read_text = lambda path, output: (
                source.replace(
                    "OmniMoltenSuperheavyUpdate", "removedSuperheavyHook"
                )
                if path == lava_path else original(path, output)
            )
            periodic_audit.check_engine(ROOT, errors)
            ...
            periodic_audit.read_text = original
        self.assertTrue(any("molten superheavy" in error for error in errors))
