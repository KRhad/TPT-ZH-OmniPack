...
$autorunSource = Join-Path $scriptRoot "runtime\omni_atmosphere_cpu_mvp.lua"
$tempParent = [System.IO.Path]::GetFullPath($TemporaryDirectory)
$testRoot = Join-Path $tempParent ("tpt-omni-atmosphere-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $resultPath = Join-Path $resolvedTestRoot "omni-atmosphere-cpu-mvp.result"
    ...
    do {
        ...
        $resultText = if (Test-Path -LiteralPath $resultPath) { [string](Get-Content -LiteralPath $resultPath -Raw) } else { "" }
        if ($resultText -match "(?m)^OMNI_ATMOSPHERE_STATUS=PASS\r?$") {
            $passed = $true
            break
        }
        if ($resultText -match "(?m)^OMNI_ATMOSPHERE_STATUS=FAIL\r?$") { break }
    } while ([DateTime]::UtcNow -lt $deadline -and -not $process.HasExited)

    if (-not $passed) {
        throw "OmniAtmosphere Lua runtime failed; result=$resultText; artifacts=$resolvedTestRoot"
    }
    Write-Output "runtime-lua-omni-atmosphere: PASS"
