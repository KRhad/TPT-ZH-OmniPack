...
$autorunSource = Join-Path $scriptRoot "runtime\environment_regression.lua"
if (-not (Test-Path -LiteralPath $autorunSource -PathType Leaf)) {
    throw "Missing Lua environment regression script: $autorunSource"
}
...
$testRoot = Join-Path $tempParent (
    "tpt-omnipack-environment-" + [guid]::NewGuid().ToString("N")
)
...
if (-not $resolvedTestRoot.StartsWith(
    ...
    throw "Refusing to create an environment test outside the temporary directory"
}
...
try {
    ...
    $result = Join-Path $resolvedTestRoot "lua-environment-regression.result"
    ...
    do {
        ...
        if (Test-Path -LiteralPath $result -PathType Leaf) {
            ...
            if ($resultText -match "(?m)^OMNI_ENVIRONMENT_STATUS=(PASS|FAIL)\r?$") {
                break
            }
        }
        $process.Refresh()
    ...
    if (
        $resultText -match "(?m)^OMNI_ENVIRONMENT_STATUS=PASS\r?$" -and
        $resultText -match "(?m)^OMNI_ENVIRONMENT_ELEMENTS=16\r?$" -and
        $resultText -match "(?m)^OMNI_ENVIRONMENT_IDS=670-685\r?$" -and
        $resultText -match "(?m)^OMNI_ENVIRONMENT_BEHAVIOURS=17\r?$" -and
        $resultText -match "(?m)^OMNI_ENVIRONMENT_EVENT_REACTED=1024\r?$" -and
        $resultText -match "(?m)^OMNI_ENVIRONMENT_EVENT_PEAK=1024\r?$"
    ) {
        $passed = $true
    }
    $responding = $false
    ...
    if (-not $passed) {
        throw (
            "Lua environment regression failed; responding=$responding; " +
            "result=$resultText; artifacts=$resolvedTestRoot"
        )
    }
    if (-not $responding) {
        throw "Lua environment regression passed but client is unresponsive"
    }
    Write-Output "runtime-lua-environment-test: PASS"
    ...
    if ($passed -and (Test-Path -LiteralPath $resolvedTestRoot)) {
        for ($attempt = 1; $attempt -le 20; $attempt++) {
            try {
                ...
                if ($attempt -eq 20) {
                    Write-Warning "Could not remove environment test directory: $resolvedTestRoot"
                }
                Start-Sleep -Milliseconds 100
            }
        }
